  ...
  <section class="training-query" aria-label="训练数据查询">
        ...
        <table class="auto-summary-table" aria-label="多人通过统计" :style="{ minWidth: `${multiTableMinWidth}px` }">
          ...
          <tbody>
            <tr v-for="row in displayRows" :key="row.row.user.username" :class="{ 'is-error': row.row.status === 'error' }">
              <th class="auto-summary-player-cell"><span class="auto-summary-player"><strong>{{ row.row.user.username }}</strong><small>{{ row.row.user.username }} · {{ row.row.summary?.authorHandle || '查询失败' }}</small></span></th>
    ...
    <template v-else-if="mode === 'single' && (submissions || firstAccepted)">
      <div class="training-stat-grid" aria-label="训练数据统计">
        <article class="training-stat-card identity-card"><span>个人信息</span><strong>{{ selectedUsername || '未选择队员' }}</strong><small>{{ acceptedSummary?.authorHandle || `未绑定 ${OJ_LABELS[selectedOjName]}` }}</small></article>
      ...
      <div class="training-result-grid">
        <article class="rating-panel">
          <header><div class="rating-panel-title"><h2>难度分布</h2></div></header>
        ...
        <article class="recent-submission-panel">
          <header><div class="activity-switch" role="tablist"><button :class="{ 'is-active': singleTab === 'submissions' }" type="button" @click="singleTab = 'submissions'">最近提交</button><button :class="{ 'is-active': singleTab === 'accepted' }" type="button" @click="singleTab = 'accepted'">最近通过</button></div></header>
    ...
    <template v-else-if="mode === 'problem' && (problemSubmissions || problemFirstAccepted)">
      ...
      <article class="recent-submission-panel problem-query-panel">
        <header><div class="activity-switch"><button :class="{ 'is-active': problemTab === 'submissions' }" type="button" @click="problemTab = 'submissions'">提交明细</button><button :class="{ 'is-active': problemTab === 'accepted' }" type="button" @click="problemTab = 'accepted'">首 AC handle</button></div></header>
...
import type { useTrainingDashboard } from '../composables/useTrainingDashboard';
...
const props = defineProps<{ dashboard: ReturnType<typeof useTrainingDashboard>; mode: TrainingQueryMode }>();
const dashboard = props.dashboard;
const { status, trainingUsers, includeRetiredUsers, selectedUsername, selectedOjName, trainingQuery,
...
function trainingUserOptionLabel(user: TrainingUser) { return `${user.username} · ${user.handles[selectedOjName.value] || `未绑定 ${OJ_LABELS[selectedOjName.value]}`}`; }
